...
function DoppelgangerStart( keys )
    ...
    local dparticle = ParticleManager:CreateParticle("particles/units/heroes/hero_phantom_lancer/phantom_lancer_doppleganger_aoe.vpcf",PATTACH_CUSTOMORIGIN,caster)
    ParticleManager:SetParticleControl(dparticle,0,caster:GetAbsOrigin())
    ParticleManager:ReleaseParticleIndex(dparticle)
...
function CheckUnits(keys)
    ...
    if target:GetUnitName() == caster:GetUnitName() and target:GetMainControllingPlayer() == caster:GetMainControllingPlayer() then
        ...
        local rand_distance = math.random(0,radius) 
        local rand_position = ability:GetAbsOrigin() + RandomVector(rand_distance)

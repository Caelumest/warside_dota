LinkLuaModifier("modifier_troy_the_arena", "heroes/casalmar/fertile_soil.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_troy_the_arena_caster", "heroes/casalmar/fertile_soil.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_troy_the_arena_check_position", "heroes/casalmar/fertile_soil.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_troy_the_arena_barrier", "heroes/casalmar/fertile_soil.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_troy_the_arena_knockback", "heroes/casalmar/fertile_soil.lua", LUA_MODIFIER_MOTION_HORIZONTAL)
LinkLuaModifier("modifier_troy_the_arena_pull", "heroes/casalmar/fertile_soil.lua", LUA_MODIFIER_MOTION_HORIZONTAL)

fertile_soil = fertile_soil or class({})

function fertile_soil:GetAOERadius()	
	return self:GetSpecialValueFor("field_radius")		
end

function fertile_soil:OnSpellStart()			
	if IsServer() then
		-- Ability properties
		...
		local duration = ability:GetSpecialValueFor("duration") + caster:GetTalentValue("special_bonus_unique_casalmar_arena_duration")
--		local vision_aoe = ability:GetSpecialValueFor("vision_aoe")
		...
		if caster:HasScepter() then
			local pct_dps_scepter = ability:GetSpecialValueFor("pct_dps_scepter")
			Timers:CreateTimer(0, function()
				...
				for _, unit in pairs(units) do
					local dps_scepter = unit:GetMaxHealth() * pct_dps_scepter
...
--		local vision_aoe = self:GetAbility():GetSpecialValueFor("vision_aoe")
		...
		local particle_field = "particles/units/heroes/hero_troy/troy_arena.vpcf" -- the field itself
...
--		AddFOWViewer(self:GetCaster():GetTeamNumber(), self.target:GetAbsOrigin(), vision_aoe, self.duration, false)
		...
		if self:GetCaster():HasScepter() then
			...
			self.lava_particle = ParticleManager:CreateParticle(lava_field, PATTACH_WORLDORIGIN, nil)
			ParticleManager:SetParticleControl(self.lava_particle, 0, self.target_point)
			ParticleManager:SetParticleControl(self.lava_particle, 1, Vector(self.field_radius, 1, 1))
			ParticleManager:SetParticleControl(self.lava_particle, 2, Vector(self.duration, 0, 0))
			ParticleManager:SetParticleControl(self.lava_particle, 4, Vector(1, 1, 1))
			ParticleManager:SetParticleControl(self.lava_particle, 15, self.target_point)  
		end
...
function modifier_troy_the_arena_check_position:kineticize(caster, target, ability)
	--if not target:HasModifier("modifier_troy_the_arena_knockback") then
		...
		elseif distance_from_border > 0 and math.abs(distance_from_border) <= 60 and target ~= caster then
			target:AddNewModifier(caster, ability, modifier_barrier, {})
...
--[[function OnTakeDamage(event)
	...
	if target:IsHero() then
		caster:RemoveModifierByName("modifier_get_attacker")
		ability:ApplyDataDrivenModifier(caster, target, "modifier_negate_damage", {Duration = 5})
	end
end

function NegateDamage(event)
	if target ~= nil and target:HasModifier("modifier_negate_damage") then

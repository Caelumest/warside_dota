...
if CasalmarResponses == nil then
    CasalmarResponses = class({})
end

function CasalmarResponses:Start(hero)
    if not CasalmarResponses.started then
        CasalmarResponses.started = true
    end
    self.hero = hero
    CasalmarResponses.responses = {}
...
function CasalmarResponses:FireOutput(outputName, data)
    if CasalmarResponses[outputName] ~= nil then
        Dynamic_Wrap(CasalmarResponses, outputName)(self, data)
    end
end

function CasalmarResponses:RegisterUnit(unitName, configFile)
    -- Load unit config
    CasalmarResponses.responses[unitName] = LoadKeyValues(configFile)
    local unitResponses = CasalmarResponses.responses[self.hero:GetUnitName()]
...
function CasalmarResponses:PlayTrigger(responses, response_rules, unit)
...
function CasalmarResponses:TriggerSound(triggerName, unit, responses)
...
function CasalmarResponses:TriggerSoundSpecial(triggerName, special, unit, responses)
    local response_rules = responses[triggerName]
    if response_rules ~= nil then
...
function CasalmarResponses:PlaySound(soundName, unit, allChat, global)
...
-- EVENT HANDLERS
--================================================================================
function CasalmarResponses:OnOrder(order)
    local unitResponses = CasalmarResponses.responses[self.hero:GetUnitName()]
...
function CasalmarResponses:OnUnitDeath(event)
    -- Unit death
    if event.unit == self.hero then
        event.unit.reincarnateflag = nil
        local unitResponses = CasalmarResponses.responses[event.unit:GetUnitName()]
    ...
    if event.attacker == self.hero then
        unitResponses = CasalmarResponses.responses[event.attacker:GetUnitName()]
...
function CasalmarResponses:OnUnitSpawn(event)
    local unit = EntIndexToHScript(event.entindex)
    local unitResponses = CasalmarResponses.responses[unit:GetUnitName()]
...
function CasalmarResponses:OnTakeDamage(event)
    local unitResponses = CasalmarResponses.responses[event.unit:GetUnitName()]
...
function CasalmarResponses:OnAbilityStart(event)
    local unitResponses = CasalmarResponses.responses[event.unit:GetUnitName()]
    print("Working ability response")
end

function CasalmarResponses:OnAbilityExecuted(event)
    local unitResponses = CasalmarResponses.responses[event.unit:GetUnitName()]
...
function CasalmarResponses:OnRunePickup(event)
...
function CasalmarResponses:OnItemBought(event)
    ...
    if GetKeyValueByHeroName(hero:GetUnitName(), "IsCustom") == 1 and hero == self.hero then
        ...
        function CasalmarResponses:GetItemName(event)
...
function CasalmarResponses:OnPlayerChat(event)
    local text = event.text
    local hero = PlayerResource:GetSelectedHeroEntity(event.playerid)
    local heroname = hero:GetUnitName()
    local unitResponses = CasalmarResponses.responses[hero:GetUnitName()]
...
function CasalmarResponses:OnItemPickup(event)
    local hero = PlayerResource:GetSelectedHeroEntity(event.PlayerID)
...
function CasalmarResponses:OnLvlUp(event)
...
function CasalmarResponses:OnGameRulesStateChange()
    ...
    if state == DOTA_GAMERULES_STATE_GAME_IN_PROGRESS then
        ...
        --for _, hero in pairs(heroes) do
            ...
            --if hero == self.hero then
                local responses = CasalmarResponses.responses[self.hero:GetUnitName()]
